...
IMPORTANT: Make sure no one is logged into either computer before proceeding.
...
. Login on the *spare* computer with *ssh*, or on a local virtual console text terminal, using an AUID account
. Execute:
+
    cd /tmp
    script refresh.txt
