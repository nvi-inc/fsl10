...
WARNING: The Government warning template in fsadapt needs to be updated. For now, edit the
script to replace the text with that contained in `govtwarn.txt` distributed with this guide.
...
*  Do not select `rcsdir` -- this will be covered later using native systemd configurations
...
=== Install startup units

// TODO: this should go in fsadapt

In the directory of this guide, copy the unit files to the system

    cp systemd/* /etc/systemd/system

Then enable them on startup:
