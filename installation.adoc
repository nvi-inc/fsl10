...
NOTE: If you do not set a locale or set `locale=C`, you will be prompted
to select your language and your country.
...
boot decice.  Then select *All files in one partition (recommended for
...
your diak, as above, and get *FREE SPACE*.
...
. Select the RAID partitions we just created using space -- these should be sda2 and
sdb2, if just have one disk, just pick sda2 -- then press kbd:[Enter]
...
rebooting or entering `systemctl restart gdm3`.
...
You can install some useful tools for working with the RAID with:
   ...
   ~/fsl10/tools_for_RAID

#TODO: BIOS version?#

The rest of this document assumes these tools have been installed. They
are:

   * `mdstat` allows all users to check on RAID status
   * `refresh_secondary` allows `root` to refresh a secondary disk
...
To be safe in all cases, this should be done even if the disk has been
used in a different FS computer or a previous install on this
computer. Two disks can be initialized at the same time.

. Start the installation process again with _only_ the disk(s) to be initialized installed.
. When you get to *Configure the network*, select *Go Back*
. Select *Detect disks*
. When you get to disk partitioning, select *Manual*
. Select the additional disk(s) to be wiped.  *DO NOT SELECT YOUR BOOT DEVICE*

.. Press kbd:[Enter] on the disk(s) to be wiped.  If you are unable to
change the area to *FREE SPACE*, please see the NOTE for step 2 in the
sub-section "Setup the first disk" in section "First Stage
Installation" above.

.. Installer may warn: *You have selected an entire device to partition…*. Select *Yes*
. Select *Configure software raid*
. Select *Yes* confirm writing changes to disk
. Select *Go Back*
. Select *Go Back*
. Select *Abort installation*
. Power down system

==== Refresh secondary disk

Boot with only the primary disk from the install turned on. The new
secondary disk must be keyed off or removed. This will ensure the
machine is booted from the good disk.

With the RAID now missing a disk, you may see 20-30s of "volume group
not found", then the machine will boot.

Once booted, login as *root*.

Key on the disk. This will ensure the secondary disk is initialized
second and is available as */dev/sdb*.

Wait until *lsblk* shows device *sdb*.

Run the refresh secondary script:
...
When the syncing is complete, you can repeat this process for a third disk.

=== Additional security and CIS Benchmarks (Optional)
...
=== Disabling direct login to oper and prog (Optional)

The model of FSL assumes oper and prog accounts will be used for operations and
programming respectively. However, some institutes may have security and
auditing restrictions that mean operators must log-in to their own account.
As the Field System currently operates, users will then need to switch to
the oper or prog account after login.

To do this, we suggest creating two groups that can `sudo` to the
...
Then add each user to these groups as appropriate, eg:
...
If the accounts haven't been disable for login already, do so:

    usermod -L oper
    usermod -L prog

To prevent connecting with `ssh` using a key, create (or add to an
existing) `DenyUser` line in `/etc/ssh/sshd_config`:
...
user `oper` but it works analogously for `prog`):

1. Add this to the following files
+
.~/.login
[source,csh]
```
...
Execute:
    ...
    chmod a+rx /usr/local/bin/switch2oper

=== Enabling a user to promote to `root` (Optional)

run `visudo` then add at end:

    dhorsley      ALL=(root) ALL

A password will still be required for promotion.

Authorized users can then switch to `root` with:

    su -i -u root

No X authority manipulation is required.

=== Network configuration changes (Optional)
...
to `Delete` any other connections *except* `Wired Connection 1`.
...
=== Disable Desktop User (Optional)

#TODO#

=== Set /usr2 owned by prog (Optional)

#TODO: Should any (all?) systems have this#

=== Remove ModemManager (Optional)
