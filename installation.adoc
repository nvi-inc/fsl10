...
shows no resyncing in progress. If there is none, shut down the
...
To do this, we suggest suggest create two groups that can sudo to the oper and
prog accounts.
...
Then add each user to these groups ass appropriate, eg:
...
So ensure X authorization works do the following:
...
```
+
2. Make the file `/usr/local/bin/fs` with contents

[source,bash]
```
#!/bin/bash
sudo -u oper XCOOKIE="$(xauth list $DISPLAY)" FS_DISPLAY_SERVER=on -i fs
```
