...
message will included instructions for kernel updates if one is available.
...
Unless you have installed other out-of-tree modules, for a normal
FSL10 installations, the only such module that would meed this
criteria is the GPIB driver GPIB driver (if it was selected in
        *fsadapt*). It will need to be recompiled (using *fsadapt*)
...
If a kernel point fails, i.e., the new kernel fails to
boot or causes other problems, you can recover as described in FSL10 RAID document
<<raid.adoc#_test_upgrade_of_fs_system_updates_or_other_significant_changes,Test
upgrade of FS - system updates - or other significant changes>> section,
if you were following that method, or from a shelf disk according to the
<<raid.adoc#__recover_from_a_shelf_disk,Recover from a shelf disk>> if not and you have a good shelf disk.
...
out-of-tree modules will also need to restored.

=== Manually checking for updates

If you do not use the weekly update script, or if you want to make
sure you have the very latest updates when you install them, you can run
the weekly update script manually to check for the latest updates:

    /root/fsl10/etc_cron.weekly_apt-show-upgradeable 

Then to install the updates, follow the update procedure in sub-section <<Installing updates>>
above, except you will use the instructions in the output of the
script above instead of from the *cron* job output (which should be
        equivalent for the same set of upgrades). 

Any NEWS items will included in the script output along with the
packages to be updated. If you would like to see any NEWS items more
distinctly before installing updates, you can rerun (or initially run)
    the script with the *--which=new* option:
