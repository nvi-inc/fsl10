...
would typically correspond to _/dev/mapper/vg0-usr2_, which would
correspond to `Path` _/dev/vg0/usr2_.
...
"`Post-check`" sub-step below, to check that the volume changed by the
expect amount and that no files were lost or changed size or
modification times.
...
if you using the FSL10 RAID system. If not, it is strongly recommended
...
in <<raid.adoc#,Raid Notes for FSL10>> document.  +
...
... Set the size to a new larger total size, say 4GB:
...
files were lost or changed size or modification times.
...
NOTE: If you are not using the FSL10 RAID system, you will need to use
...
<<raid.adoc#,Raid Notes for FSL10>> document.
