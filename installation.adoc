...
displayed, you may need to use *Guided partitioning* to create new
partitions. You may be prompted to delete RAID andi/or
LVM, which you must do. Then switch back to *Manual* and select the
target disk again. That should get you back to just *FREE SPACE*.
...
change the area to *FREE SPACE*, please see the NOTE for step 2 in
section 3.11.1.
...
Center for Internet Security (CIS), move on to the <<cis-setup2.adoc#,CIS hardening FSL10>> document.
...
Create (or add to an existing) `DenyUser` line in
`/etc/ssh/sshd_config`:
...
Authorized users can then switch to `oper` and `prog` with:
