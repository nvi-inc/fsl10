...
you have an Internet connection) and the installation of \~197 new packages
(unless you did not select the Desktop or added other tasks earlier -
currently downloading \~188MB from the Internet and/or DVDs).
...
If you did not select the GPIB option in the previous page deselec the
...
those services you actually need and press kbd:[Enter] on *OK* to set
them up and finish with FSADAPT. If you need printing, you will need
to select "netipp". However remote  access to this can be blocked by
configuring `ufw` and either not explicitly allowing or deny the CUPS
service.
...
Edit the `~prog/.profile` file uncomment `export FS_SERIAL_CLOCAL=1`
line. Log-out and log back in as `prog`.
...
=== Enabling user promotion to `oper`, `prog` and `root` (Optional)
...
If a user can elevate to `root`, also add:
...
=== Generate FQDN in HELO for outgoing mail (Optional)
...
message, you can correct this by commenting out the following lines
...
Where `FQDN` is your system's Fully Qualified Domain Name. The restart the server:
