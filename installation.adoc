...
TIP: More information about RAID operation can be found at <<raid.adoc#,RAID notes for FSL10>>.
...
message, there are two possibke solutions, "recommended" and "alternate".

A defect of both approaches is that they hard codes the *HELO* message
  data, which will have to be manually updated if the FQDN changes.  A
  more general solution may be possible, but hasn't been discovered
  yet.


==== Recommended method

The recommended  way to fix this a solution is to add the following
line to */etc/exim4/update-exim4.conf.conf*:

....
MAIN_HARDCODE_PRIMARY_HOSTNAME=<FQDN>
....
...
A possible alternate solution is to comment out the following lines in
