...
If updates are needed, the *cron* job will send a message to *root*
(or whoever e-mail to *root* is aliased to, typically *oper*) with
instructions on how to install the updates. You can choose a convenient
time, when not in (or about to start) operations, to apply the
updates and test the system.

Kernel point updates, for example from *9.11* to *9.12*, require extra
care and testing. They are described in more detail in the next
sub-section. You can tell if a point upgrade is needed if the *cron*
job includes the package *linux-image-amd64* (or *linux-image-i386*
        depending on your architecture).  Non-kernel point updates are
less risky.

TIP: It is recommended that a disk rotation be performed before any
update. This will make recovery much easier if a problem with the
update is discovered.  Please see the FSL10 Raid document for
information on performing disk rotations and how to manage testing
updates.

=== Kernel point updates

By default the instructions from the *cron* job will cause kernel
point upgrades to be installed. If it is desired, these can be
deferred to a later date when more extensive testing can be performed
by using *apt-get* in place of *apt* in instructions.

If you are using the GPIB driver (as selected in *fsadapt*). It will
need to be recompiled (using *fsadapt*) after a kernel point update and
then the *ibcon* will need to be reloaded (as *prog*):
...
If a kernel point upgrade fails, i.e., the previous kernel fails to boot or
causes other problems, the previous kernel can be used. For a single
boot, use the *Advanced* option in the *grub* menu at boot and then
select the previous kernel. You can change back permanently to the previous
...
The *-12* would be the later version that should be purged:

    apt purge linux-image-4.9.0-12-amd64

Likewise with the linux-headers. For this there will be two point
version packages you should delete:

    linux-headers-4.9.0-12-amd64
    linux-headers-4.9.0-12-common

The GPIB driver will need to be recompiled and *ibcon* reloaded
as described above.
