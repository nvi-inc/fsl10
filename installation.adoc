...
kernel updates. The *cron* download job output will include a warning
at the end with additional instructions depending on which is
available.  The two types are:

. Point updates, e.g., from *9.11* to *9.12*, which change the kernel
ABI (Application Binary Interface). The warning for this case is:
...
. Non-point updates, which update the kernel, but do not change the
...
install these updates.  As described in point update warning, you will need to rebuild
any out-of-tree modules after rebooting for that case. This is discussed in the
<<Additional kernel point update steps>> sub-section below.
...
If an update fails, e.g., an updated kernel fails to boot or other problem is discovered,
...
according to the <<raid.adoc#_recover_from_a_shelf_disk,Recover from
a shelf disk>> if not and you have a good shelf disk.

==== Additional recovery option for failed kernel point update

For a kernal point update that has failed, it is also possible to try to use the
