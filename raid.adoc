...
. Key-on the _secondary_ slot when prompted
. Answer `*y*` to blank
. Once the refresh is complete, you have recovered to the state of the previous _good_ _shelf_ disk.
...
rotation you should move the _secondary_ the shelf instead of the
_primary_.
...
For safety reasons, to ensure that only an old _shelf_ disk is overwritten,
any _secondary_ disk (_sdb_) will be rejected that:
...
This script can be used to drop a _primary_ disk (_sda_) out of a RAID pair
...
. Use of the _mdadm_ command with `-f` option to mark it as faulty.
