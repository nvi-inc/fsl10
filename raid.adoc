...
content the script intends to copy is where it expects it to be and
has the right form.  Any *primary* disk (*sda*) will be rejected that:
...
pair of disks that were most recently used _together_ in an active
...
*failed* if it was either marked _failed_ by hand or due to disk errors.
It is *missing* if either the system was rebooted with the disk
*failed* or *missing* or it was manually marked _removed_ by hand.  You
...
been changed in any way e.g., by being used (booted) separately (which is
    caught be the script) or refreshed against some other disk, or if
...
*inactive* disk is relatively fresh. You can check the progress with *mdstat*. The
...
this state, it should use the working disk. At the first opportunity,
     ...
     erase the new disk when it is introduced into the rotation
     sequence.
...
*metserver* and *metclient* as it would in the former scenario; this
may be significant time saver.
